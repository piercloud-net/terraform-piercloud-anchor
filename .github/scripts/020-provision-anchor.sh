...
#              CSR on EVERY outcome + tang thumbprint to artifact paths,
...
ROTATE=0
# Artifact path for the per-anchor Origin CA CSR (public material; test
# harnesses override it so they never write into the checkout).
ORIGIN_CA_CSR_OUT="${ORIGIN_CA_CSR_OUT:-./origin-ca.csr}"
...
# --- csr-capture:start --- (tests/origin-ca extracts this span; keep markers)
# The per-anchor CSR is public material generated by 010 section (c1) BEFORE
# every probe runs, so it is captured on EVERY outcome: a failed 010 is
...
run_onbox_provision() { # run 010 over SSH, capture the CSR on every outcome, return the 010 status
  local provision_rc=0
  if [ "$ROTATE" -eq 1 ]; then
    { echo "$ENV_PREFIX"; cat scripts/010-provision.sh; } | ssh_base 'bash -s -- --rotate' || provision_rc=$?
  else
    { echo "$ENV_PREFIX"; cat scripts/010-provision.sh; } | ssh_base 'bash -s' || provision_rc=$?
  fi
  if [ "$provision_rc" -ne 0 ]; then
    # Best-effort on the error path: a missing/failed capture must never
    # mask the original 010 failure.
    capture_origin_ca_csr || warn "per-anchor CSR not captured on the failed run — the original 010 failure stands"
    return "$provision_rc"
  fi
  capture_origin_ca_csr || die "per-anchor Origin CA CSR capture failed — refusing to publish an unverifiable artifact"
  return 0
}
# --- csr-capture:end ---
...
cmd_provision() {
  ...
  # Runs 010 and captures the CSR on every outcome; on a 010 failure this
  # returns that status (set -e exits the script with it) AFTER the capture.
  run_onbox_provision
