...
# Issue #121: tmp policy names are piercloud-tmp-<run_id> — the server id
# left the name (it is the alias → netcup-account join key). Fixtures use
# the new shape; the legacy this-server shape is scoped by the real
# list_tmp_policies filter (tests/policy-naming).
...
RC_OF() { local l="$1" mode="${3-apply}"; STUB_LIST="$2" STUB_FAIL_DETACH_PID="${STUB_FAIL_DETACH_PID:-}" PATH="$WORK/bin:$PATH" STUB_ACTIONS="$WORK/actions.$l" GITHUB_STEP_SUMMARY="$WORK/summary.$l" MODE="$mode" SERVER_ID=933556 OWN_NAME="piercloud-tmp-999" TMP_TTL_SECONDS=7200 \
...
rc=$(RC_OF c2 "[$(entry 11 piercloud-tmp-1 60)]")
is "c2 rc" "0" "$rc"
is "c2 actions" "detach 11 delete 11" "$(ACTIONS c2)"
is "c2 leaked log" "$(grep -c "retiring leaked tmp policy 'piercloud-tmp-1'" "$WORK/out.c2" || true)" "1"

# ---- case 3: two fresh foreign tmps -> BOTH retired, run proceeds -----------
rc=$(RC_OF c3 "[$(entry 11 piercloud-tmp-1 60),$(entry 12 piercloud-tmp-2 120)]")
is "c3 rc" "0" "$rc"
...
rc=$(RC_OF c4 "[$(entry 11 piercloud-tmp-1 10800),$(entry 12 piercloud-tmp-2 9000)]")
is "c4 rc" "0" "$rc"
...
rc=$(RC_OF c5 "[$(entry 11 piercloud-tmp-1 10800),$(entry 12 piercloud-tmp-2 60)]")
is "c5 rc" "0" "$rc"
...
rc=$(RC_OF c6 "[$(entry 11 piercloud-tmp-1 orphan)]")
is "c6 rc" "3" "$rc"
...
rc=$(RC_OF c7 "[$(entry 99 piercloud-tmp-999 30),$(entry 12 piercloud-tmp-2 60)]")
is "c7 rc" "0" "$rc"
is "c7 actions" "detach 12 delete 12 detach 99 delete 99" "$(ACTIONS c7)"

# ---- case 8: own leftover alone -> dropped ---------------------------------
rc=$(RC_OF c8 "[$(entry 99 piercloud-tmp-999 30)]")
is "c8 rc" "0" "$rc"
...
rc=$(RC_OF c9 "[$(entry 11 piercloud-tmp-1 10800),$(entry 12 piercloud-tmp-2 orphan)]")
is "c9 rc" "3" "$rc"
...
rc=$(RC_OF c10 "[$(entry 11 piercloud-tmp-1 60),$(entry 12 piercloud-tmp-2 120),$(entry 13 piercloud-tmp-3 10800)]")
is "c10 rc" "0" "$rc"
...
rc=$(RC_OF c11 "[$(entry 11 piercloud-tmp-1 60)]")
...
rc=$(RC_OF c12 "[$(entry 99 piercloud-tmp-999 30),$(entry 98 piercloud-tmp-999 45)]")
is "c12 rc" "0" "$rc"
...
rc=$(RC_OF c13 "[$(entry 11 piercloud-tmp-1 60),$(entry 99 piercloud-tmp-999 30)]" check)
is "c13 rc" "0" "$rc"
...
rc=$(RC_OF c14 "[$(entry 11 piercloud-tmp-1 orphan)]" check)
is "c14 rc" "3" "$rc"
...
rc=$(RC_OF c15 "[$(entry 11 piercloud-tmp-1 60)]" "")
is "c15 rc" "0" "$rc"
