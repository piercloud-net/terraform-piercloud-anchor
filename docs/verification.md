...
- [ ] CI green; the `recording-witness` harness renders the real witness span, proves the strict-list-only property (signed `ListObjectsV2`/`ListMultipartUploads` only, pagination followed for both list families, and **every** scenario's request SigV4-verified server-side) and the verdicts against the mock endpoint: healthy, `heartbeat-missing`/`heartbeat-stale`, `recording-gap` (shell, legacy, and the conservative in-flight-exec shape that clears once the `.exec` end lands), `session-end-missing` (completed tar + lost end), `completer-lag`, `sequence-gap`/`sequence-duplicate`/`sequence-origin`, `session-start-missing` (incl. the pre-fold sid-bearing `session.rejected` regression shape), `open-upload-stale`, `naming-contract` (shape-based drift + sid-less `session.rejected`), `contract-mismatch`, exec-vs-shell mode markers (the `session.end` marker is authoritative — live v18 marks only the end; legacy shape = conservative shell; duplicate ends **and** duplicate starts resolve by the newest `LastModified` in both listing orders), clock skew → `error` (future `LastModified` **and** future multipart `Initiated`), malformed/error-document/truncated list responses → `error`, verdict-log rotation, install/disable (verdict log kept), run-once `ExecMainStatus` mapping + the `InvocationID`/per-run-identity (`run_seq`) advancement anchor (wedged start, failed state write, same-second double run, >300 s run, state/status mismatch), and ntfy transition/recovery/renotify bookkeeping including first-ever-green silence and the failed-recovery retry across the same-second boundary. Chain fixtures are built with the pc-admin shipper key grammar (`tests/recording-witness/shipper_keys.py`, pinned to `cad0p/pc-admin` @ `66bd304` with golden strings + unexpected-shape refusal teeth + the checked-in golden/boundary vector matrix generated from the real builder — uppercase-sid lowercasing, multi-segment `session.*` → `unknown`, 7+/18-digit seq bounds, seq 0 hand-written legacy, incl. the forced-sid-less `session.rejected` regression tooth).
