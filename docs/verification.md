...
2. Run `tools/verification-browser/approve-device.sh <run-id>`: it resolves the device URL from `--device-url` / `DEVICE_URL`, then ntfy (`--ntfy-topic` / `NTFY_TOPIC`, optional `--ntfy-token` / `NTFY_TOKEN`), then the notice annotation (all lookups bounded), pre-flights the profile, navigates, confirms the Keycloak Grant Access page, clicks `#kc-login`, and polls for `/realms/scp/device/status` + `Device Login Successful` → prints `DEVICE_LOGIN_SUCCESSFUL`.
