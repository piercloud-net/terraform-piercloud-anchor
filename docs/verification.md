...
- [ ] CI green; the `recording-witness` harness renders the real witness span, proves the strict-list-only property (signed `ListObjectsV2`/`ListMultipartUploads` only, pagination followed for both list families, and **every** scenario's request SigV4-verified server-side) and the verdicts against the mock endpoint: healthy, `heartbeat-missing`/`heartbeat-stale`, `recording-gap` (shell, legacy, and the conservative in-flight-exec shape that clears once the `.exec` end lands; an uppercase-sid tar or in-progress upload still satisfies the lowercased session's gap check), `session-end-missing` (completed tar + lost end), `completer-lag`, `sequence-gap`/`sequence-duplicate`/`sequence-origin`, `session-start-missing` (incl. the pre-fold sid-bearing `session.rejected` regression shape), `open-upload-stale`, `naming-contract` (shape-based drift + sid-less `session.rejected`; a replay-conflict variant `session.rejected_<hash16>` is not drift), `contract-mismatch`, replay-conflict variants absorbed as one event identity (base + `_<sha256[:16]>` variant on start/data/end → ok, no `sequence-duplicate`; the base key stays authoritative for the lifecycle mode even when the variant is newer and even when the variant is listed first (non-conformant fixture order); the identity tuple excludes the mode marker and drives sequence counting only, so a same-(ts, seq) `.exec`/`.shell` marker pair resolves both keys — the newest `LastModified` wins with its mode and a conflicting equal-`LastModified` tie fails closed to `shell` in both listing orders (a stale `.exec` cannot exempt by sorting first); a mode marker on a variant shape is `naming-contract` drift exactly like the plain shape; a genuine same-seq duplicate at a different ts still alerts `sequence-duplicate`; a variant-only session start stays conservative `session-start-missing`), exec-vs-shell mode markers (the `session.end` marker is authoritative — live v18 marks only the end; legacy shape = conservative shell; duplicate ends **and** duplicate starts resolve by the newest `LastModified` in both listing orders, and an exact equal-`LastModified` tie with conflicting modes resolves conservatively to `shell`), clock skew → `error` enforced at collection time over every listed object (future `LastModified` incl. exec sessions and completed tars, **and** future multipart `Initiated`), malformed/error-document/truncated list responses → `error`, corrupt-state handling (non-numeric counter → error verdict + push + repair with the readable baseline held; an unreadable, oversized (> 1 MiB **bytes** — the file is read in binary, so a multi-byte state over the byte bound is invalid input even under the character count; the suite pins the multi-byte case), invalid-UTF-8 (the binary read names the encoding failure), or deeply nested `state.json` → bounded read + `.corrupt` preserve (never overwriting an existing `.corrupt`; timestamped sibling fallback) + repaired `baseline: null` (the record is marked `repaired`) and names the systemd `INVOCATION_ID` that wrote it, instead of an uncaught `RecursionError`/`MemoryError`; a failed preservation (unwritable state dir) warns with the actual destination it tried), planted-symlink refusal for `state.json.tmp`/`verdict.log` (the victim is never touched), an orphan completed tar with no audit events → `session-start-missing` past the grace, verdict-log rotation, install/disable (verdict log kept), run-once `ExecMainStatus` mapping + the `InvocationID`/per-run-identity (`run_seq`) advancement anchor (wedged start, failed state write, same-second double run, >300 s run, state/status mismatch, an explicit this-invocation `repaired` record accepted as progress even when `run_seq` resets to 1 while a stale repair marker from a previous invocation still dies), and ntfy transition/recovery/renotify bookkeeping including first-ever-green silence, the failed-recovery retry across the same-second boundary, a future last-notify epoch, caught `BadStatusLine`/`IncompleteRead` push failures, a bad/oversized token that skips the push (state + verdict still land, the token is never echoed), and refused HTTP redirects on both the ntfy push and the signed S3 list GETs (no `Authorization` header is ever re-sent cross-origin). Chain fixtures are built with the pc-admin shipper key grammar (`tests/recording-witness/shipper_keys.py`; `a7035a9` is the grammar-defining pin — builder grammar last changed there (the replay-conflict `_<sha256[:16]>` variant keys from `disambiguate_audit_key`); the previous grammar points were `66bd304` (session.rejected sid-less), `929d82c` (the 128-char pre-hash event-type truncation cap), `41735ff` (the `_<sha256[:8]>` collision-resistant suffix on the truncated type) and `342a37c` (the `10^18−1` seq-ceiling clamp in `build_audit_key`) — with golden strings + unexpected-shape refusal teeth + the checked-in golden+boundary+variant vector matrix generated from the real builder and a `source_sha`-provenance assertion that demands the exact 40-hex pin (a short prefix, a prefix plus junk, and longer prefix-sharing hex all fail): uppercase-sid lowercasing, multi-segment `session.*` → `unknown`, 7+/18-digit seq bounds, seq 0 hand-written legacy, over-long-type truncation + hash suffix (incl. a shared-head collision pair and an invalid over-long refusal), replay-conflict variants for session.start/session.end (mode marker dropped), session.rejected/user.login (hash joins the last type segment) and an over-long truncated type (truncation suffix kept, conflict suffix appended), incl. the forced-sid-less `session.rejected` regression tooth).
