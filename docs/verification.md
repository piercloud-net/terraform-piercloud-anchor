...
- [ ] CI green; the `recording-witness` harness renders the real witness span, proves the strict-list-only property (signed `ListObjectsV2`/`ListMultipartUploads` only, pagination followed for both list families, and **every** scenario's request SigV4-verified server-side) and the verdicts against the mock endpoint: healthy, `heartbeat-missing`/`heartbeat-stale`, `recording-gap`, `session-end-missing` (completed tar + lost end), `completer-lag`, `sequence-gap`/`sequence-duplicate`/`sequence-origin`, `session-start-missing`, `open-upload-stale`, `naming-contract` (shape-based drift + sid-less `session.rejected`), `contract-mismatch`, exec-vs-shell mode markers (the `session.end` marker is authoritative — live v18 marks only the end; legacy shape = conservative shell), clock skew → `error`, malformed/error-document/truncated list responses → `error`, verdict-log rotation, install/disable (verdict log kept), run-once `ExecMainStatus` mapping + stale-state refusal, and ntfy transition/recovery/renotify bookkeeping. Chain fixtures are built with the pc-admin shipper key grammar (`tests/recording-witness/shipper_keys.py`).
